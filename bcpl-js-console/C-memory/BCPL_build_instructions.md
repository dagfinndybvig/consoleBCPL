...
- On some platforms/builds the historical build used additional flags; if the simple commands above fail, try:

```sh
gcc -O2 -std=c99 -Wall -Wextra -Dstricmp=strcasecmp -DO_BINARY=0 -include unistd.h -o C_icint icint.c
```

The `-Dstricmp=strcasecmp` and `-include unistd.h` flags help on systems where `stricmp`/`strcasecmp` and POSIX headers are not found by default.
