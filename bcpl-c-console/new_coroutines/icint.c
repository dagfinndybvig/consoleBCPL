...
/* --- CHANGECO debug tracing flag --- */
/* Set to 1 to enable stderr tracing of all CHANGECO calls */
static int co_debug = 1;
...
short m[WORDCOUNT], lomem, himem = WORDCOUNT - 1, cis, coutfd, sysin, sysprint, vecfree;
    ...
    case F6_K: d += sp;
      if (a < PROGSTART) {
        ...
        switch (a) {
          ...
          case K17_OUTPUT: a = coutfd; goto fetch;
          ...
          case K90_CHANGECO: {
            ...
            if (co_debug) {
              fprintf(stderr, "CHANGECO enter: arg=%d cptr=%d currco_addr=%d currco=%d sp=%d pc=%d\n",
                arg, cptr, currco_addr, currco, sp, pc);
              fprintf(stderr, "  cptr fields: [0]sp=%d [1]pc=%d [2]parent=%d [3]next=%d [4]fn=%d [5]size=%d [6]self=%d\n",
            ...
            if (currco != 0) {
              ...
              m[currco + 1] = pc;
              if (co_debug) {
                fprintf(stderr, "  saved currco[%d]: sp=%d pc=%d\n", currco, sp, pc);
              }
            }
            ...
            if (co_debug) {
              fprintf(stderr, "CHANGECO exit: new_sp=%d new_pc=%d a=%d\n", sp, pc, arg);
            }
...
fn0(void, init) {
  ...
  cis = sysin = STDIN_FILENO + 1;
    coutfd = sysprint = STDOUT_FILENO + 1;
}
